...
@st.cache_resource
def load_model():
    model = tf.keras.models.load_model('model.h5')  # type: ignore # Ensure 'model.h5' is in your repo
    return model
...
if uploaded_file is not None:
    ...
    # Display result
    result = f"🔴 Stroke Detected with {round((float(prediction) * 100), 2)}% Confidence"   if prediction >= 0.5 else "🟢 No Stroke Detected"
    roundd = round((prediction * 100), 2)
